@props(['label' => '', 'spinner' => false, 'href' => false, 'icon' => false, 'red' => false])
<div>
    @if ($href)
        <a href="{{ $href }}"
            {{ $attributes->class([
            ...
            @switch($icon)
                ...
                @case('download')
                    <svg class="text-white size-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
                        height="24" fill="none" viewBox="0 0 24 24">
                ...
                @case('whatsapp')
                    <svg class="size-5 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
    ...
    @else
        <button
            ...
            @endswitch
            {{ $label }}
            <x-siakad.spinner class="size-4" wire:loading wire:target="{{ $spinner }}" />
        </button>
    @endif
</div>
