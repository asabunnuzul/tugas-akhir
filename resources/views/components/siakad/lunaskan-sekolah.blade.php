...
x-on:open-lunaskan-sekolah-modal.window="showModal = true; pembayaranId = $event.detail.pembayaranId"
x-on:close-lunaskan-sekolah-modal.window="showModal = false"
class="fixed inset-0 z-30 flex items-center justify-center overflow-auto bg-white bg-opacity-10" x-show="showModal">
    ...
    <div class="max-w-3xl px-6 py-4 mx-auto text-left bg-white rounded-md shadow-md" @click.away="showModal = false"
        ...
        <!-- content -->
        <div class="py-5">
            Anda Yakin Mengatur Beasiswa Pembayaran ini ?
            <div class="flex space-x-3">
                <button type="button"
                    class="z-50 p-2 text-white bg-emerald-500 border rounded-md cursor-pointer border-slate-100 hover:bg-emerald-600 focus:ring focus:ring-emerald-600 hover:ring hover:ring-emerald-600"
                    @click="$wire.sekolah(pembayaranId), showModal = false">
                    Ya, Atur Beasiswa
                </button>
                <button type="button"
